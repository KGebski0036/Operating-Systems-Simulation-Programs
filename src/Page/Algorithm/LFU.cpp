...
void LFU::startAlgorithm(std::vector<Page>& pages, std::string& fileToSaveOutput, size_t capacity) {
	capacity_ = capacity;
	...
	for (size_t i = 0; i <= pages.size(); i++) {
		...
		if (missing != 0) {
			if (memory_.size() < capacity_)
			{
				...
				auto it = std::min_element(pagesUsedCounter.begin(), pagesUsedCounter.end());
...
void LFU::printFooter(size_t numberOfSubstitutes) {
	std::cout << E << YELLOW << "Number of substitutes: " << numberOfSubstitutes << E;
}

void LFU::printStep(size_t needed, size_t missing) {
...
void LFU::saveToFile(std::vector<Page>& pages, std::string& fileToSaveOutput) {
